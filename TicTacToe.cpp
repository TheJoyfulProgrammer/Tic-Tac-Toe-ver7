...
namespace t3
{
	...
	{
		// Pick a random element to retrieve an unplayed position from the remaining available positions.
		int ArrayPointer = std::experimental::randint(1, t3::BoardPositionsRemaining) - 1;

		// Play the position chosen
		SetPosition(t3::PlayableBoardPositions[ArrayPointer], Player);

		// Delete the element in the vector array that was just chosen.
